...
### Cross-checking against FIFA's official squad-list PDF

`pipeline/fifa_squad_lists_2026.pdf` (downloaded from fdp.fifa.org, not
regenerated by any script) is a reference source, not a pipeline input.
`pipeline/check_fifa_pdf_consistency.py` parses it (via `pdftotext -layout`)
and cross-checks it against `wc2026_players.csv`/`wc2026_coaches.csv`,
matching players within each nation by date of birth (the two sources don't
share a join key, and the PDF's "SURNAME Firstname" format doesn't
string-match our CSV's "Firstname Surname" scrape). It's a standalone
diagnostic, not part of the core build sequence — run it manually:
